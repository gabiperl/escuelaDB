...
try {
    ...
    if (isset($_POST['nombre']) && isset($_POST['apellido']) && isset($_POST['email']) && isset($_POST['departamento_id']) && isset($_POST['rol']) && isset($_POST['fecha_ingreso'])) {
        $nombre = $_POST['nombre'];
        ...
        $sql = "INSERT INTO MiembrosDepartamento (Nombre, Apellido, Email, Telefono, DepartamentoID, Rol, FechaIngreso)
                VALUES (:nombre, :apellido, :email, :telefono, :departamento_id, :rol, :fecha_ingreso)";
        ...
        header('Content-Type: application/json');
        echo json_encode(array("error" => "Por favor, proporciona todos los datos requeridos del miembro mediante una petición POST (nombre, apellido, email, departamento_id, rol, fecha_ingreso). El teléfono es opcional."));
    }
